...
fig <- df %>%
  ...
  add_lines(x = ~DATE, y = ~W398RC1A027NBEA, name = 'Original', line = list(color = 'blue')) %>%
  add_lines(x = ~DATE, y = ~MA2, name = 'MA(2)', line = list(color = 'red', dash = 'dash')) %>%
  add_lines(x = ~DATE, y = ~MA4, name = 'MA(4)', line = list(color = 'green', dash = 'dash')) %>%
  add_lines(x = ~DATE, y = ~MA8, name = 'MA(8)', line = list(color = 'purple', dash = 'dash')) %>%
  layout(title = "Moving Average Smoothing For Household Income",
...
```{r, echo=FALSE}
library(plotly)
htmlwidgets::saveWidget(fig, "../../../image/EDA/income.html")
```
